...
이 객체는 private final LinkedHashMap<Class<? extends SecurityConfigurer<O, B>>, List<SecurityConfigurer<O, B>>> configurers;

즉, configure들을 가지고 있어서 configure 각각의 init()과 configure()를 호출한다.
